...
<div class="min-h-screen bg-gradient-to-br from-slate-900 via-gray-900 to-slate-900">
    ...
    <div class="relative z-10 p-6">
        <!-- Page Header -->
        <div class="mb-8">
            <h1 class="text-3xl font-light text-white mb-2">Edit Review</h1>
                ...
                <form method="POST" action="{{ route('admin.reviews.update', $review) }}">
                    ...
                    <div class="mb-8">
                        ...
                        <div class="space-y-6">
                            ...
                            <div>
                                ...
                                <div class="flex space-x-4">
                                    <label class="flex items-center space-x-2 cursor-pointer">
                                        <input type="radio" name="customer_type" value="existing" {{ (old('customer_type', $review->user_id ? 'existing' : 'custom')) == 'existing' ? 'checked' : '' }}
                                    ...
                                    <label class="flex items-center space-x-2 cursor-pointer">
                                        <input type="radio" name="customer_type" value="custom" {{ (old('customer_type', $review->customer_name ? 'custom' : 'existing')) == 'custom' ? 'checked' : '' }}
                                            class="w-4 h-4 bg-gray-900/70 border border-gray-700 rounded focus:ring-amber-500/50 focus:border-amber-500/50">
                            ...
                            <div id="custom-customer-section" class="customer-section {{ $review->customer_name ? '' : 'hidden' }}">
                                <label for="customer_name" class="block text-sm font-light text-gray-300 mb-2">Customer Name *</label>
                                <input type="text" id="customer_name" name="customer_name" value="{{ old('customer_name', $review->customer_name) }}" placeholder="Enter customer name"
                            ...
                            <div>
                                ...
                                <select id="car_id" name="car_id" required
                                    ...
                                    @foreach($cars as $car)
                                        <option value="{{ $car->id }}" {{ (old('car_id', $review->car_id)) == $car->id ? 'selected' : '' }}>
                            ...
                            <div>
                                ...
                                <div class="flex items-center space-x-4">
                                    <div class="flex space-x-1">
                                        @for($i = 1; $i <= 5; $i++)
                                            <button type="button" class="star-btn text-2xl {{ (old('rating', $review->rating)) >= $i ? 'text-amber-400' : 'text-gray-600' }}" data-rating="{{ $i }}">
                                                <i class="fas fa-star"></i>
                                            </button>
                                        @endfor
                                    </div>
                                    <span class="text-gray-400" id="rating-display">{{ old('rating', $review->rating) }}/5</span>
                            ...
                            <div>
                                ...
                                <textarea id="comment" name="comment" rows="4" maxlength="1000"
                                    class="w-full px-4 py-3 bg-gray-900/70 border border-gray-700 rounded-xl text-white placeholder-gray-400 focus:outline-none focus:border-amber-500/50 transition">{{ old('comment', $review->comment) }}</textarea>
                                <div class="text-xs text-gray-400 mt-1"><span id="comment-count">0</span>/1000 characters</div>
                            </div>

                            <div class="flex items-center space-x-3">
                                <input type="checkbox" id="is_verified" name="is_verified" value="1" {{ old('is_verified', $review->is_verified) ? 'checked' : '' }}
                    ...
                    <div class="flex justify-end space-x-4">
                        <a href="{{ route('admin.reviews.show', $review) }}" class="px-6 py-3 bg-gray-700/50 text-gray-300 rounded-xl font-light hover:bg-gray-700 hover:text-white transition-colors">
                            Cancel
                        </a>
